...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll("[data-nav]").forEach((el) => {
    el.addEventListener("click", (e) => {
      e.preventDefault();
      showView(el.dataset.nav.split("-")[0]); // "forgot-1" -> "forgot"
    });
  });

  // Password show/hide toggles (data-toggle-target="<input id>")
  ...
  // Land on dashboard if already signed in, otherwise login.
